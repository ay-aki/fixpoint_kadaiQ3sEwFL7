...
"""
アピールポイント：
1. 外部ライブラリを使って、短めのコードにした。
   基本的に外部ライブラリの信頼性について疑わずにコーディングしている。
   (pandasやnetaddrなどの誤動作のハンドリングなどは行わない)
2. データの処理は、入力、処理、出力で完全に分離し、分析系統のライブラリを用いている。
   (データストリームのような逐次的な処理は行っていない
3. ネットワークアドレスの計算にはnetaddrライブラリを使用
"""
...
def main(argv):
    ...
    # problem 2 故障状態(only num_in_failure >= N)
